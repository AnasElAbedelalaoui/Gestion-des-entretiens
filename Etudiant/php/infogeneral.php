					...
					<div class= "titre" style=" text-align: center">
						<h1>
							<?php
								...
								echo"<h3>";
								echo"	Mes informations générales :";
								echo"</h3>	";
								echo"<h4>	";
								...
								$sql = ' SELECT `idEtu`,`nomEtu`,`prenomEtu`,`emailEtu`,`telEtu`,`noAdrEtu`,`rueAdrETu`,`cpAdrEtu`,`villeETu` FROM `Etudiant` WHERE `nomEtu`="'.$_POST["nom"].'" and prenomEtu="'.$_POST["prenom"].'"';
							
								
								$result = mysqli_query($conn, $sql) or die("Requête invalide: ".mysqli_error()."\n".$sql);
									
								while ($row = mysqli_fetch_array($result)) { 
									echo 'Identifiant : '.$row[0].' <br/>';
									echo 'Nom :'.$row[1].' <br/>';
									echo 'Prenom :'.$row[2].' <br/>';
									echo 'Email :'.$row[3].' <br/>';
									echo 'Numéro de téléphone :'.$row[4].' <br/>';
									echo '<li> Adresse :'.$row[5].': '.$row[6].' '.$row[8].' ('.$row[7].')<br/></li>';
